...
$$\begin{aligned}
\argmax_→w p(→t|⇉X; →w) =& \argmin_→w ∑_{i=1}^N -\log p(t_i | →x_i ; →w) \\
...
~~~
Given that $→y(→x_A)_k$ was the largest among $→y(→x_A)$ and also
given that $→y(→x_B)_k$ was the largest among $→y(→x_B)$, it must
be the case that $→y(→x)_k$ is the largest among all $→y(→x)$.
...
| logistic regression | $σ(x)$ | Bernoulli | $\textrm{NLL} ∝ 𝔼 -\log(p(t \vert →x))$ | $\big(a(y(→x)) - t\big) →x$ |
| multiclass<br>logistic regression | $\small\operatorname{softmax}(x)$ | categorical | $\textrm{NLL} ∝ 𝔼 -\log(p(t \vert →x))$ | $\big(a(y(→x)) - →1_t\big) →x^T$ |
